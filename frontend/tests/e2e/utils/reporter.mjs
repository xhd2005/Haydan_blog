// frontend/tests/e2e/utils/reporter.mjs
import { getAssertionCount } from './assertions.mjs';
...
export class Reporter {
  ...
  recordTest(testResult) {
    ...
    if (status === 'PASS') {
      ...
      if (error) {
        ...
        if (error.stack && (this.verbose || process.env.DEBUG)) {
  ...
  endSuite() {
    if (this.currentSuite) {
      ...
      console.log(`${colors.dim}Suite Duration: ${suiteDuration}ms | Passed: ${statusColor}${passed}${colors.dim}, Failed: ${failed > 0 ? colors.red + failed : '0'}, Skipped: ${skipped}${colors.reset}\n`);
    }
  }
  ...
  printSummary() {
    ...
    console.log(`${colors.bright}              E2E TEST SUITE EXECUTION SUMMARY (VISIONOS CMS)         ${colors.reset}`);
    ...
    for (const tier of tiers) {
      ...
      if (tierTests.length > 0) {
        ...
        console.log(` ${colors.bright}${tier.padEnd(8)}:${colors.reset} Total: ${String(tierTests.length).padStart(3)} | Passed: ${color}${String(tPassed).padStart(3)}${colors.reset} | Failed: ${tFailed > 0 ? colors.red : colors.dim}${String(tFailed).padStart(2)}${colors.reset} | Skipped: ${String(tSkipped).padStart(2)}`);
      }
    }

    console.log(`${colors.dim}──────────────────────────────────────────────────────────────────────${colors.reset}`);
    console.log(` ${colors.bright}Overall  :${colors.reset} Total: ${String(total).padStart(3)} | Passed: ${colors.green}${String(passed).padStart(3)}${colors.reset} | Failed: ${failed > 0 ? colors.red : colors.dim}${String(failed).padStart(2)}${colors.reset} | Skipped: ${String(skipped).padStart(2)}`);
    console.log(` ${colors.bright}Asserts  :${colors.reset} ${colors.cyan}${totalAssertions}${colors.reset} verified assertions across all tiers`);
