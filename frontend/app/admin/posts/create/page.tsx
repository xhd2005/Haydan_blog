...
export default function CreatePostPage() {
  ...
  // 发布属性抽屉展开状态 (默认折叠以确保 100% 专注正文；点击右上角一键展开)
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('draft_post_new');
      if (saved) {
      ...
      console.error('Failed to parse draft_post_new', e);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      localStorage.setItem('draft_post_new', JSON.stringify(payload));
      const now = new Date();
  ...
  }, [title, slug, excerpt, content, categoryId, selectedTagIds, lang]);
  ...
  const handleSubmit = async (submitStatus = status) => {
    ...
    try {
      ...
      toast.success(submitStatus === 'PUBLISHED' ? '文章已成功发布上线！' : '草稿已保存！');
  ...
  return (
    <div className="w-full space-y-5">
      {/* 统一规范头部 */}
      <AdminPageHeader
        title="新建博文 (Markdown Studio)"
        description="沉浸式创作工坊：支持全景禅写作、双栏实时对照、本地草稿防丢、MinIO 拖拽直传与一键派生双语译文。"
        icon={PenTool}
        breadcrumbs={[
