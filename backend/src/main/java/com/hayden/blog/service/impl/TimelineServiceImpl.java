...
import com.hayden.blog.mapper.TimelineMapper;
...
public class TimelineServiceImpl extends ServiceImpl<TimelineMapper, Timeline> implements TimelineService {
    ...
    @Override
    public TimelineSpan getTimelineSpan() {
        List<Timeline> timelines = getAllTimelines();
        if (timelines.isEmpty()) {
            return new TimelineSpan(null, null, 0);
        }
        // Java 25 Sequenced Collections getFirst() & getLast()
        return new TimelineSpan(timelines.getFirst(), timelines.getLast(), timelines.size());
    }
}
